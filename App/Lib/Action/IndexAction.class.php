...
class IndexAction extends Action {
    ...
    public function index() {
        ...
        $view['id']=$id;
        if(isset($_GET['id'])){
        if(!$m->where($view)->select()){
            $this->redirect("http://music.jnu.fm");
        }
        }

        $view_count=$m->where($view)->getField('view_count');
		$where['id']=$id;
        $where['view_count']=$view_count+1;
        $m->save($where);
        ...
        $content['sid']=$id;
        $con=$n->where($content)->order("count desc")->getField('content');
        
        
        $this->assign('comment',$con);
        ...
        if(isset($_GET['type'])){
            ...
            $this->display('onPhone');
        }elseif(!isset($_GET['id'])){
            //   $m=D('music');
            //    $hotsong=$m->hotsong();
            //     $newsong=$m->newsong();
            //    $this->assign('hot',$hotsong);  
            //      $this->assign('new',$newsong);
            // //   dump($newsong);
            // //  dump($hotsong);
            // //   exit;
            // $this->display('default');
            $song1   = $m->where("state=2")->select();
            $song2   = $m->where("state=1")->select();
            $song3   = $m->where("state=3")->select();
            $newsong = $m->where("state=4")->select();
            ...
            $arr=$m->find($id);
            $songlist =$m->getHotList();
            $this->assign('data',$arr);
            $this->assign('songlist',$songlist);
            $comment = D('Comment');
            $findcom['sid'] = $id;
            $cs = $comment->where($findcom)->select();
            $this->assign("comment",$cs);
    ...
    //使四项数据相应+1
     public function add($id,$type) {
         ...
         switch($type){
             case 1:$t='dl_count';break;
             case 2:$t='play_count';break;
             case 3:$t='vote_count';break;
         }
         ...
         /*   if($type==3)
         {
            $n=D('vote');  
             ...
             $arr=$n->where($v)->getField('state');
             if($arr['state']==1)                 //到时换成第二期或第三期直接来这里改变state的值，，同时改数据库中state的默认值
                {                
                 $music=$D('music');
                $vote['id']=$id;
       ...
       {$m=D('music');
         $where['id']=$id;
         $count=$m->where($where)->getField($t);
    ...
    public function random($id){
        ...
        { $i=rand($minid,$maxid);
            ...
            $c=$m->where("id=$i AND id!=$id")->find();          
             if($c>0)
               $flag=1;
        }         
        $this->redirect("Index/index",array('id'=>$i));
            
    }
    
    
    //上一首或下一首
      public function shunxu($id,$type) {
        ...
        $this->redirect("Index/index",array('id'=>$next_id));
            
    }
         
    
    //返回JSON形式的评论列表
    public function comment_return(){
        ...
        //     $toid=$_POST['toid'];
        //     $id=$n->where("id=$toid")->order("id desc")->getField('id');
        
       
        //     $count=$n->where("id=$id")->getField('count');
    ...
    public function getlist(){
        $music = D("music");
        $this->ajaxReturn($music->getlist(),"搜索成功",0);
    }

    public function search(){
        $this->checkGet(array('keyword'));

        $music = D("music");
        if ( empty($_GET['keyword'])) {
            $this->ajaxReturn(null,"参数为空",3);
        }
        $result = $music->search(urldecode($_GET['keyword']));
        $count  = count($result);

        if($count == 0 ){
            $this->ajaxReturn(null,"没有搜索到结果",1);
        }

        $data['data']  = $result;
        $data['count'] = $count;

        $this->ajaxReturn($data,"搜索成功",0);
    }
    ...
    public function commentlist(){
        ...
        $this->ajaxReturn($comment->getlist($_GET['sid']),'返回数据呵呵呵呵呵呵呵呵呵呵呵呵呵呵呵呵呵呵',0);
    }
    ...
    public function commentadd(){
        ...
        $data['sid']     = $_GET['sid'];
        $data['number']  = count($comment->where($data)->select()) + 1;//获取用户
        $data['count']   = 0;
        $data['content'] = urldecode(htmlspecialchars($_GET['content']));//url传过来的参数，为gb编码，需要转换为utf8
        $data['toid']    = $_GET['toid'];

        //if(isset($_GET))
        if(!$comment->create($data))
            $this->ajaxReturn(null,$comment->getError(),1);
        ...
        if(isset($_GET['toid'])){
            ...
            $data2['id'] = $data['toid'];
            $comment->where($data2)->setInc('count');

        }
        $this->ajaxReturn($comment->getlist($data['sid']),"添加成功",0);
            
    }
    //获得最热门微博
    
    private function checkGet($arr){
        foreach ($arr as $value) {
            if (!isset($_GET[$value])) {
                $this->ajaxReturn(null,$value."变量为必选参数",1);
            }
        }
    }
    ...
    public function sidget(){
        $m=D('music');
        $data['no'] = $_GET['no'];
        $result = $m->where($data)->find();
        if(!$result )
            $this->ajaxReturn(null,"编号不存在",1);
        else
            $this->ajaxReturn($result,"获取成功",0);
